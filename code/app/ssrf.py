...
def process(opt):
	...
	validateLocalHost_regex = r"^http://localhost/[a-zA-Z0-9-_]+/[a-zA-Z0-9-_]+\.php$"

	# Validating Host name
	# if not(re.match(validateHost_regex,args.host) or re.match(validateHostIpWithPort_regex,args.host) or re.match(validateLocalHost_regex, args.host)):
	# 	t = "Terminating... Please enter Host in the format http://google.com or https://google.com or http://10.10.10.10 for internal hosts"
	# 	print(t)
	# 	exc_output.append(t)
	# 	sys.exit()
	#
	# if args.payload and not re.match(validateHost_regex,args.payload) and not re.match(validateHostIpWithPort_regex,args.payload) and not re.match(validateLocalHost_regex, args.payload):
	# 		t = "Terminating... Please enter Host in the format http://google.com or http://192.168.1.1:80"
	# 		print(t)
	# 		exc_output.append(t)
	# 		sys.exit()
	...
	def makingExternalRequests(paramName, url):
		...
		t = "\033[91m[+] Making external request with the potential vulnerable url:"+formingPayloadURL
		print(t)
		info_output.append(t)
	...
	def matchURLKeywordsInName(getOrForm,paramName,url):
		...
		if temp not in ssrfVul and re.search(matchList,paramName,re.I):
			t = "\033[92m[-] Potential vulnerable '{}' parameter {} '{}' at '{}'".format(getOrForm,"Name",paramName,url)
			print (t)
			info_output.append(t)
			ssrfVul.add(temp)
	...
	def matchURLPatternInValue(getOrForm, paramName,paramValues,url):
		...
		if temp not in ssrfVul and (re.match("^(http:\/\/www\.|https:\/\/www\.|http:\/\/|https:\/\/)?[a-z0-9]+([\-\.]{1}[a-z0-9]+)*\.[a-z]{2,5}(:[0-9]{1,5})?(\/.*)?$",paramValues) or re.match("((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)(\.|$)){4}",paramValues)):
			t = "\033[92m[-] Potential vulnerable '{}' parameter {} '{}' at '{}'".format(getOrForm, "Value" if paramName=="" else "Name",paramValues if paramName=="" else paramName,url)
			print (t)
			info_output.append(t)
			ssrfVul.add(temp)
			if args.payload and getOrForm == "GET":
	...
	baseURL = parsed.scheme + "://" + parsed.netloc + parsed.path
	print ("Target URL - " + baseURL)
	info_output.append("Target URL - " + baseURL)
